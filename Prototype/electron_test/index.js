...
const TEMPLATES_DIR = IS_PACKAGED ? path.join(APP_ROOT, 'templates') : path.join(__dirname, '..', '..', 'templates');
...
ipcMain.handle('pick-template', async (_, detName) => {
  const templatesDir = TEMPLATES_DIR;
  const result = await dialog.showOpenDialog({
    title: `Pick template for ${detName}`,
  ...
  // Auto-downscale to 1080p base if captured at higher resolution
  const { ASPECT_RATIOS } = require('./src/config');
  ...
  const monH = display.size.height;
  const scale = monH / ar.sampleH;

  if (scale > 1.05) {
    // Downscale template to 1080p base
    try {
      const { execSync } = require('child_process');
      const tmpPath = filePath !== destPath ? filePath : destPath;
      if (filePath !== destPath) fs.copyFileSync(filePath, destPath);
      execSync(`python -c "
import cv2
scale = ${scale}
img = cv2.imread(r'${destPath.replace(/\\/g, '\\\\')}')
h, w = img.shape[:2]
img = cv2.resize(img, (int(w/scale), int(h/scale)))
cv2.imwrite(r'${destPath.replace(/\\/g, '\\\\')}', img)
"`, { timeout: 5000 });
      log(`Template auto-downscaled to 1080p base (/${scale.toFixed(2)}x)`);
    } catch (e) {
      crashLog(`Template downscale failed: ${e.message}`);
      if (filePath !== destPath) fs.copyFileSync(filePath, destPath);
    }
  } else {
    if (filePath !== destPath) fs.copyFileSync(filePath, destPath);
...
ipcMain.handle('start-vision', async () => {
  ...
  try {
    visionProcess = IS_PACKAGED
      ? spawn(WORKER_PATH, [configPath, templatesDir], { stdio: ['pipe', 'pipe', 'pipe'] })
      : spawn('python', [WORKER_PATH, configPath, templatesDir], { stdio: ['pipe', 'pipe', 'pipe'] });
...
ipcMain.handle('capture-game-screen', async () => {
  ...
  try {
    const cmd = IS_PACKAGED
      ? `"${CAPTURE_SCRIPT}" "${configPath}" "${outPath}"`
      : `python "${CAPTURE_SCRIPT}" "${configPath}" "${outPath}"`;
...
ipcMain.handle('open-zone-editor', () => {
  ...
  editorWin.loadFile('zone-editor.html');
});

ipcMain.handle('set-debug-threshold', (_, name, value) => {
  if (config && config.detectables[name]) {
    config.detectables[name].threshold = value;
    config.save();
    // Forward to running Python worker for live update
    if (visionProcess && visionProcess.stdin.writable) {
      visionProcess.stdin.write(JSON.stringify({ cmd: 'set_threshold', name, value }) + '\n');
    }
